...
- **Date / Timestamp**: `2026-09-13T14:00:00+05:30`
...
| **Shipped-Observation Reads** | **16** | 16 image documents in dataset read from content-addressed cache (15 resolved amounts) |
...
| **Execution Latency** | **~5.5 s** | 250 requests evaluated (~22 ms / request) |
...
```bash
# 1. Live Evidence Rebuild (re-extracts all images and verifies message facts via gpt-6-astra)
python code/main.py --rebuild-evidence --live --dataset /path/to/dataset

# 2. Live Adaptive Investigation for a single request
python code/main.py --investigate request_26 --live --dataset /path/to/dataset

# 3. Deterministic Replay (default, offline, zero network calls)
python code/main.py --mode full --output output.csv --dataset /path/to/dataset
```
